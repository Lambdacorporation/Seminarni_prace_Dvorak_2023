...
class L1(customtkinter.CTkFrame):
    def __init__(self, master, values):
        ...
        #/home/pi/krizovatky/krizovatka6.jpg
        my_image = customtkinter.CTkImage(dark_image=Image.open("/home/pi/krizovatky/krizovatka6.jpg"), size=(400, 400))
        image_label = customtkinter.CTkLabel(master, image=my_image, text="")
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.title = Title(self, "Křižovatka 5", values=["option 1", "option 2", "option 3"])
        ...
        self.radiobutton_frame = MyRadiobuttonFrame(self, "Options", values=["První pojede růžové auto, po něm modré auto a nakonec zelené auto", "První pojede zelené auto, druhé růžové auto a naposled pojede modré auto", "První pojede modré auto současně se zeleným, a nakonec pojede růžové auto"])
    ...
    def button_callback(self):
        ...
        for radiobutton in self.radiobutton_frame.radiobuttons:
            if radiobutton.cget("value") == "První pojede modré auto současně se zeleným, a nakonec pojede růžové auto":
        ...
        if selected_option == "První pojede růžové auto, po něm modré auto a nakonec zelené auto":
            self.button.configure(text=button_new_text, command=self.o1_l1)
        elif selected_option == "První pojede zelené auto, druhé růžové auto a naposled pojede modré auto":
            self.button.configure(text=button_new_text, command=self.o2_l1)
        elif selected_option == "První pojede modré auto současně se zeleným, a nakonec pojede růžové auto":
            self.button.configure(text=button_new_text, command=self.o3_l1)
    def o1_l1(self):
        try:
            subprocess.run(["python", "6s1.py"])
            time.sleep(5)
            subprocess.run(["python", "6s1b.py"])
    ...
    def o3_l1(self):
        try:
            subprocess.run(["python", "6c.py"])
            time.sleep(5)
            subprocess.run(["python", "6cb.py"])
        except Exception as e:
            print(f"Chyba při spouštění skriptu: {e}")
    def menu_port(self):
